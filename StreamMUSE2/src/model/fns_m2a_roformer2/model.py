...
from ..pl_tokenzier_base_model.model import PlBaseModel
from .config import FnsM2ARoformer2Config
from .model_io import FnsM2ARoformer2Input
...
class FnsM2ATransformer2(PlBaseModel):
    def __init__(self, config: FnsM2ARoformer2Config):
    ...
    def training_step(self, batch: FnsM2ARoformer2Input, batch_idx):
    ...
    def validation_step(self, batch: FnsM2ARoformer2Input, batch_idx):
        batch = self._move_to_device(batch)
        x_mel, x_acc, pitch_shift = batch.mel_data, batch.acc_data, batch.pitch_shift
        loss = self.loss(x_mel, x_acc, pitch_shift)
    ...
    def _move_to_device(self, batch: FnsM2ARoformer2Input) -> FnsM2ARoformer2Input:
        """
