...
class RoFormerEncoder(nn.Module):
    ...
    def forward(
        ...
        if interleave_pos:
            ...
            pos_ids = ((torch.arange(seq_len, device=hidden_states.device).unsqueeze(0).expand(B, -1) - 1) // 2).clamp(min=0)
            pos_emb = self.embed_positions.weight[pos_ids]  # [B, seq_len, head_dim]
            sinusoidal_pos = pos_emb.unsqueeze(1).expand(B, 1, seq_len, -1)
            # # Print results to inspect interleaving
            # print("Sequence length:", seq_len)
            # print("pos_ids (shows mapping frame index for each token position):")
            # print(pos_ids[0].tolist())
            # print("\nCorresponding positional embeddings (indexes in embed_positions_weight):")
            # for idx, frame_id in enumerate(pos_ids[0].tolist()):
            #     print(f"Token position {idx}: frame index = {frame_id}, embedding = {pos_emb[0, idx].tolist()}")
...
#     "RoFormerForCausalLM",
#     "RoFormerForMaskedLM",
#     # "RoFormerForMultipleChoice", ** We don't support multiple choice yet.
...
__all__ = [
    "CustomedRoformerNetwork",
    "CustomedRoformerConfig",
]
