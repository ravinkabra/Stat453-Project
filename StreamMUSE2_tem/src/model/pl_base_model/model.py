...
class PlBaseModel(L.LightningModule):
    def __init__(self, config: UnionModelConfig):
        ...
        if config.training_probing_training_logger_config:
            training_probing_schema = config.training_probing_training_logger_config
        else:
            # Fallback or raise error if schema is critical
            logger.error("training_probing_training_logger_schema is missing in BaseModelConfig. Using default values.")
            training_probing_schema = TrainingProbingLoggerConfig()  # Use a default
    ...
    def _record_abnormal_event(self, current_loss: torch.Tensor, global_step: int, avg_loss: float, batch: PlBaseModelInput):
        ...
        if self.trainer.is_global_zero:
            ...
            batchs_to_save = {name: tensor.cpu() for name, tensor in batch_dict.items() if isinstance(tensor, torch.Tensor)}
