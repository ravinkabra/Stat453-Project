...
class OldPtDataset(Dataset):
    def __init__(self, config: OldPtDatasetSchema):
        ...
        try:
            ...
            self.pitch_shift_range_mel = torch.load(self.file_path.replace("acc.pt", "mel.pt")[:-3] + ".pitch_shift_range.pt", mmap=True).reshape(
                -1, 2
            )
...
class OldPtDataModule(pl.LightningDataModule):
    ...
    def _collate_fn(self, batch: list[M2AModelInputData]) -> M2AModelInputData:
        ...
        return M2AModelInputData(mel_data=mel_data, acc_data=acc_data, pitch_shift=pitch_shift_tensor)
