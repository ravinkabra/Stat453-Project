...
# Key detection and C-major conversion helpers for realtime-ish inference.
...
def continuation(
    ...
    inference_root = "temp_cmajor_rt"
    c_mel_path, c_acc_path, key_name, shift_to_c = make_c_major_pair(midi_path, inference_root)
    print(f"[C-major-RT] Inference input key: {key_name}, shift_to_c={shift_to_c:+d}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="OldPtSFSTransformer realtime-ish inference script")
    ...
    parser.add_argument("--inference_interval", type=int, default=1, help="number of frames to generate each step")
