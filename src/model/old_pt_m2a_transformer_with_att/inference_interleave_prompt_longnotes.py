...
def continuation(
    ...
    # 指定时间步的 duration 变长
    t_start = prompt_length - 16  # 你想修改的时间步
    for t in range(t_start, prompt_length):
        max_polyphony = byte_arr_mel[0].shape[1] // 3
        for p in range(max_polyphony):
            duration_idx = p * 3 + 2
            if byte_arr_mel[0][t, duration_idx] == 255:
                continue
            # 变长（第 11 个位置，duration = 64 ticks = 4 bar）
            new_duration = 11
            byte_arr_mel[0][t, duration_idx] = new_duration
    ...
    if prompt_length != 0:
        decode_output(
            ...
            f"temp/{model.save_name}/{os.path.basename(midi_path_prompt)}_promptlen{prompt_length}.mid",
            tempo=90.0,
        )
    ...
    with torch.no_grad():
        ...
        # padding for long notes in prompt
        vec = torch.tensor([3203, 3204, 3204, 3204, 3204, 3204, 3204, 3204], device=x.device)
        batch_size = x.shape[0]
        target = vec.unsqueeze(0).unsqueeze(0)  # 变成 [1, 1, 8]
        target = target.expand(batch_size, 64, 8)  # 广播到 [batch_size, 64, 8]

        x_mel_gt = torch.cat([target, x_mel_gt], dim=1)
    ...
    for i in range(n_samples):
        ...
        decode_output(
            output_i,
            f"temp/{model.save_name}/prompt{prompt_length}/{os.path.basename(midi_path_gt)}_temp{temperature}_{i}.mid",
            tempo=90.0,
        )
