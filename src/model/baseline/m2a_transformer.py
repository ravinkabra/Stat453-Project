...
from src.network.customized_roformer.network import CustomizedRoFormerEncoder
...
class RoFormerSymbolicTransformer(L.LightningModule):
    ...
    def get_base_model(self, config):
        return CustomizedRoFormerEncoder(config)
    ...
    def local_encode(self, x, token_type_ids):
        ...
        # prepend SOS:
        x = torch.cat(
            [torch.full((x.shape[0], 1), SOS_TOKEN, dtype=torch.long, device=x.device), x], dim=-1
    ...
    def preprocess(
        ...
        if y == None:
            ...
            y_processed = torch.zeros(
                batch_size_y, seq_length_y, subseq_length_y // 3, 2, dtype=torch.long, device=y.device
            )
...
class FramedDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        pitch_shift_range_c = torch.load(
        ...
        while True:
            ...
            for i in range(0, len(self.valid_indices), self.batch_size):
                ...
                starts = (
                    torch.floor(torch.rand(len(raw_ids)) * (self.length[raw_ids] - self.target_length)).long()
                    + self.start[raw_ids]
                )
...
def sanity_check():
    ...
    x = [
        [[0, 34, 12, EOS_TOKEN, PAD_TOKEN, PAD_TOKEN], [12, 34, 52, 34, 12, EOS_TOKEN], [12, 34, 52, 34, 12, 52]],
        [
    ...
    y = [
        [[42, 34, 12, EOS_TOKEN, PAD_TOKEN, PAD_TOKEN], [22, 34, 52, 34, 12, EOS_TOKEN], [22, 34, 52, 34, 12, 52]],
        [
...
if __name__ == "__main__":
    ...
    train_set_loader = DataLoader(
        FramedDataset(dataset, TRAIN_LENGTH, batch_size, split="train"), batch_size=None, num_workers=0
    )
    val_set_loader = DataLoader(
        FramedDataset(dataset, TRAIN_LENGTH, batch_size, split="val"), batch_size=None, num_workers=0
    )
    ...
    trainer = L.Trainer(
        ...
        logger=WandbLogger(
            name=model_name,
            project="StreamMUSE",
            config={"batch_size": batch_size, "model_size": model_size, "train_length": TRAIN_LENGTH},
        )
        if args.wandb
        else TensorBoardLogger("tb_logs", name=model_name),
