...
if __name__ == '__main__':
    ...
    default_name = f"m2a_transformer_v0.3_{model_size}_batch_{batch_size * len(n_gpus)}_schedule"
    ...
    torch.cuda.memory._dump_snapshot("before_train.pickle") # memory snapshot before training
    trainer.fit(net, train_set_loader, val_set_loader, ckpt_path=checkpoint_path)
    torch.cuda.memory._dump_snapshot("after_train.pickle") # memory snapshot after training
