...
class BaseTrainerConfig:
    ...
    devices: Union[List[int], str, int] = Field(default="auto", description="使用的设备")
    ...
    # 日志和记录器
    logger: Union[Any, List[Any], bool, None] = Field(default=True, description="日志记录器")
    log_every_n_steps: Optional[int] = Field(default=50, description="每N步记录一次日志")

    # 回调和插件
    callbacks: Union[List[Any], Any, None] = Field(default=None, description="训练回调")
    plugins: Union[Any, List[Any], None] = Field(default=None, description="Lightning插件")

    # 快速开发和调试
    fast_dev_run: Union[int, bool] = Field(default=False, description="快速开发运行 (运行n批次或1批次)")
    ...
    # 过拟合测试
    overfit_batches: Union[int, float] = Field(default=0.0, description="过拟合测试批次 (int=批次数, float=比例)")

    # 验证控制
    val_check_interval: Union[int, float, None] = Field(default=1.0, description="验证检查间隔 (int=步数, float=比例)")
    check_val_every_n_epoch: Optional[int] = Field(default=1, description="每N个epoch进行一次验证")
    num_sanity_val_steps: Optional[int] = Field(default=2, description="完整性验证步数")

    # 检查点和进度条
    enable_checkpointing: Optional[bool] = Field(default=True, description="是否启用检查点")
    enable_progress_bar: Optional[bool] = Field(default=True, description="是否启用进度条")
    enable_model_summary: Optional[bool] = Field(default=True, description="是否启用模型摘要")

    # 梯度累积和裁剪
    accumulate_grad_batches: int = Field(default=1, description="梯度累积批次数")
    gradient_clip_val: Union[int, float, None] = Field(default=None, description="梯度裁剪值")
    gradient_clip_algorithm: Optional[str] = Field(default="norm", description="梯度裁剪算法 ('value' 或 'norm')")

    # 确定性和性能
    deterministic: Union[bool, Literal["warn"], None] = Field(default=None, description="确定性模式")
    benchmark: Optional[bool] = Field(default=None, description="性能基准模式")
    inference_mode: bool = Field(default=True, description="推理模式")

    # 分布式采样器
    use_distributed_sampler: bool = Field(default=True, description="是否使用分布式采样器")
    ...
    # 数据加载器重载
    reload_dataloaders_every_n_epochs: int = Field(default=0, description="每N个epoch重新加载数据加载器")

    # 目录和注册表
    default_root_dir: Union[str, Any, None] = Field(default=None, description="默认根目录")
