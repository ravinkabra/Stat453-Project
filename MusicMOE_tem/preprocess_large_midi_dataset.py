...
def create_npy_dataset_from_midi(folder, max_polyphony, dataset_name, ins_ids='all', scan_subfolders=True, max_idx=50000):
    '''
    ...
    for i in os.listdir(folder):
        for j in os.listdir(os.path.join(folder, i)):
            if j.endswith('.MID') or j.endswith('.mid'):
                midi_files.append(os.path.join(os.path.join(folder, i),j))
    if max_idx is not None:
        midi_files = midi_files[:max_idx]
    ...
    os.makedirs('data/aria16/', exist_ok=True)
    torch.save(torch.cat(results_data, dim=0), f'data/aria16/{dataset_name}.pt')
    torch.save(torch.cat(results_shift, dim=0), f'data/aria16/{dataset_name}.pitch_shift_range.pt')
    lengths = [len(data) for data in results_data]
    # np.save(f'data/{dataset_name}.length.npy', np.array(lengths))
    torch.save(torch.tensor(lengths), f'data/aria16/{dataset_name}.length.pt')
