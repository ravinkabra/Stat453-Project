...
import wandb
import tempfile
print("Python TMPDIR:", tempfile.gettempdir())
TRAIN_LENGTH = 384
MAX_STEPS = 1000000
...
class RoFormerSymbolicTransformer(L.LightningModule):
    ...
    def __init__(self, large=False, moe=False, num_experts=8,topk=2,expert_capacity_factor=1.5):
        super().__init__()

        self.hidden_size = 768 if large else 768
        self.num_layers = 12 if large else 12
        self.num_attention_heads = 12 if large else 12
        self.intermediate_size = 3072 if large else 3072
        self.moe_intermediate_size = 3072 if large else 1024
        self.local_model_num_layers = 3
        self.local_model_num_attention_heads = 8 if large else 8
        self.local_model_intermediate_size = 768 if large else 768
        self.moe = moe
    ...
    def global_sampling(self, x, max_seq_len=384, temperature=0.7):
        batch_size, seq_len, subseq_len = x.shape
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-size', type=str, help='model size')
    parser.add_argument('--checkpoint-path', type=str,default=None, help='checkpoint path')
    parser.add_argument('--moe',action='store_true', help='use moe')
    parser.add_argument('--expert_num',type=int,default=8)
    parser.add_argument('--topk',type=int,default=2)
    args = parser.parse_args()
    assert args.model_size in ['small', 'large']
    # n_gpus = max(torch.cuda.device_count(), 1)
    n_gpus=1
    
    wandb.init(
        project="MoE-Music-Model-2Expert",  # 项目名称（在 wandb 官网显示）
    )
    if args.moe:
        model_name = f'cp_transformer_moe_{args.model_size}_batch_{args.batch_size * n_gpus}_expert_{args.expert_num}_topk_{args.topk}_moe_hidden{4096}_swiglu_new'
    else:
        model_name = f'cp_transformer_dense_{args.model_size}_batch_{args.batch_size * n_gpus}'
    net = RoFormerSymbolicTransformer(args.model_size == 'large', args.moe,num_experts=args.expert_num,topk=args.topk,expert_capacity_factor=100)
    train_set = FramedDataset('data/LA/50000/LA_medium.pt', TRAIN_LENGTH, args.batch_size, split='train')
    val_set = FramedDataset('data/LA/50000/LA_medium.pt', TRAIN_LENGTH, args.batch_size, split='val')
    train_indices = FramedDataset('data/LA/50000/LA_medium.pt', TRAIN_LENGTH, args.batch_size, split='train').valid_indices
    val_indices = FramedDataset('data/LA/50000/LA_medium.pt', TRAIN_LENGTH, args.batch_size, split='val').valid_indices
    assert set(train_indices.tolist()) & set(val_indices.tolist()) == set(), "训练集和验证集存在数据泄露！"
    train_set_loader = DataLoader(train_set, batch_size=None, num_workers=1, persistent_workers=True)
    val_set_loader = DataLoader(val_set, batch_size=None, num_workers=1, persistent_workers=True)

    checkpoint_callback = L.callbacks.ModelCheckpoint(monitor='val_loss',
                                                      save_top_k=1,
                                                      save_last=True,
                                                      dirpath=f'/ephemeral/ckpt/{model_name}',
    ...
    trainer = L.Trainer(devices=n_gpus,
                        ...
                        logger=WandbLogger(project="MoE-Music"),
                        strategy='auto' if n_gpus == 1 else 'ddp')
    trainer.fit(net, train_set_loader, val_set_loader, ckpt_path=args.checkpoint_path)
    # save the model (parameters only)
    torch.save(net.state_dict(), f'/ephemeral/ckpt/{model_name}.pt')