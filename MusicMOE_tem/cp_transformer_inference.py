...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 6:
        print('Usage: python cp_transformer_inference.py <model_path> <large> <moe> <expert> <topk>')
        exit(1)
    model_path = sys.argv[1]
    large = sys.argv[2].lower() == 'true'
    moe = sys.argv[3].lower() == 'true'
    expert = int(sys.argv[4])
    topk = int(sys.argv[5])
    model = RoFormerSymbolicTransformer.load_from_checkpoint(model_path, large=large,moe=moe,num_experts=expert,topk=topk)
    model.save_name = os.path.basename(model_path)
    model.cuda()
    model.eval()
    for i in range(15,16):
        i=10
        continuation(model, 'input/ashover1.mid', temperature=i*0.1, generation_length=384, n_samples=2, prompt_length=75)
        continuation(model, 'input/ashover2.mid', temperature=i*0.1, generation_length=384, n_samples=2, prompt_length=75)
        continuation(model, 'input/ashover5.mid', temperature=i*0.1, generation_length=384, n_samples=2, prompt_length=75)
        continuation(model, 'input/Ode-To-Joy.mid', temperature=i*0.1, generation_length=384, n_samples=2, prompt_length=75)
        continuation(model, 'input/RM-P003.SMF_SYNC.MID', temperature=i*0.1, generation_length=384, n_samples=2,prompt_length=75)
        continuation(model, 'input/LA_random1.mid', temperature=i*0.1, generation_length=384, n_samples=2,prompt_length=75)
        continuation(model, 'input/LA_random2.mid', temperature=i*0.1, generation_length=384, n_samples=2,prompt_length=75)
        continuation(model, 'input/LA_random3.mid', temperature=i*0.1, generation_length=384, n_samples=2,prompt_length=75)